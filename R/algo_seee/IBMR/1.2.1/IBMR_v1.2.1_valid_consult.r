...
# Fichiers lies     : IBMR_params_transcode.csv
# Commentaires 	  	: Indice Biologique Macrophytique en Riviere calcule selon
# la norme NF T90-395 (octobre 2003). Il repose sur 327 taxons pris en compte
# dans le calcul.
...
resultat <- initResult() %>%
  ...
  funColonnes(
    ...
    cols   = c("CODE_OPERATION", "CODE_STATION", "DATE",
               "CODE_TAXON", "UR", "POURCENTAGE_FACIES", "RESULTAT"),
    result = .)
  ...
  resultat <- resultat %>%
    funVide(
      Table    = select(data_entree,
                        ID, CODE_OPERATION, CODE_TAXON, UR, 
                        POURCENTAGE_FACIES, RESULTAT),
      result   = .)    %>%
    funEspace(
      Table    = select(data_entree,
                        ID, CODE_TAXON, UR, POURCENTAGE_FACIES, RESULTAT),
      result   = .)    %>%
    funCodes(
      Table    = select(data_entree, ID, UR),
      codes    = c("FU", "F1", "F2"),
      codeType = "unite de releve",
      result   = .)    %>%
    funIntervalle(
      Table    = select(data_entree, ID, POURCENTAGE_FACIES, RESULTAT),
      mini     = 0,
      maxi     = 100,
      result   = .)
}
