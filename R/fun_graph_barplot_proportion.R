...
barplot_proportion <- function(data,
  ...
  data_pct <- data_filtre2 %>%
    ...
    dplyr::summarise(n_classe = dplyr::n_distinct(code_station), .groups = "drop") %>%
  ...
  g1 <- ggplot2::ggplot(data_pct, ggplot2::aes(x = annee, y = pct, fill = libelle_classe)) +
    ...
    scale_fill_manual(values = palette_etat, 
  ...
  distrib <- patchwork::wrap_plots(g1, 
                                   g2, 
                                   ncol = 1, 
                                   heights = c(2.5, 1.5))
  
  ggplot2::ggsave(
    filename = file.path(output_dir, paste0("proportions_classe_",reseau_titre,".jpg")),
    plot = distrib,
    width = 14,
    height = 9,
    dpi = 300
...
barplot_nombre <- function(data,
  ...
  ggplot2::ggsave(
    filename = file.path(output_dir, paste0("nb_station_classe_",reseau_titre,".jpg")),
    plot = g3,
    width = 13,
    height = 7,
    dpi = 300
